...
int checkWin(char** grid, char symbol) {
    ...
    for (int i = 0; i < 3; i++) {
        if (grid[i][0] == symbol && grid[i][1] == symbol && grid[i][2] == symbol) return 1; // Check rows
        if (grid[0][i] == symbol && grid[1][i] == symbol && grid[2][i] == symbol) return 1; // Check columns
    }

    // Check diagonals
    if (grid[0][0] == symbol && grid[1][1] == symbol && grid[2][2] == symbol) return 1; // Check diagonal
...
int main() {
    ...
    while (1) {
        ...
        int row, col;
        printf("Player %c, enter your move (row and column): ", (currentPlayer == 0) ? playerX.symbol : playerO.symbol);
        scanf("%d %d", &row, &col);
